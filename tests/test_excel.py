...
def test_export_products_creates_expected_template(tmp_path, session):
    session.add(Category(name="Makanan", active=True))
    session.add(Unit(name="PCS"))
    session.commit()
    session.add(Product(
        barcode="8990001", name="Biskuit", category_id=1, unit_id=1,
    ...
    assert list(sheet.iter_rows(values_only=True))[0] == tuple(PRODUCT_HEADERS)
    assert list(sheet.iter_rows(values_only=True))[1][0] == "8990001"
    workbook.close()


def test_import_products_add_and_update_preserves_running_stock(tmp_path, session):
    session.add(Category(name="Minuman", active=True))
    session.add(Unit(name="BOTOL"))
    session.commit()
