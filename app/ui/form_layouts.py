...
def _delete_product(page):
    ...
    answer = QMessageBox.question(
        page,
        "Hapus Produk",
        f"Hapus permanen {name}?\n\nProduk yang sudah memiliki histori penjualan, pembelian, atau stok tidak dapat dihapus.",
...
def _attach_product_excel(page):
    ...
    for button in (export_button, import_button, template_button):
...
def _popup_complex_page(page, title, dialog_title, description):
    ...
    if title == "Data Produk":
        delete_button = QPushButton("Hapus Produk")
...
def _popup_master_page(page, title, dialog_title):
    ...
    if form_box is None:
        return
    _remove_widget_from_layout(page.layout(), form_box)
    controller = _PopupController(page, form_box, dialog_title,
        "Form lengkap dibuka sebagai popup agar tabel data tetap menjadi fokus utama.", width=620, height=390)
    page._wpos_form_popup = controller
...
def _attach_reset_controls(window):
    ...
    if box is None or box.layout() is None:
        return
    separator = QLabel("RESET DATA"); separator.setObjectName("sectionTitle")
    warning = QLabel("Fitur ini menghapus data secara permanen. Akun pengguna dan pengaturan toko tetap dipertahankan.")
    warning.setWordWrap(True); warning.setObjectName("pageSubtitle")
    tx_button = QPushButton("RESET TRANSAKSI & STOK"); tx_button.setObjectName("secondary")
    all_button = QPushButton("RESET SEMUA DATA BISNIS"); all_button.setObjectName("danger")
...
def _reset_transactions(window):
    if not _confirm_reset(window, "Reset Transaksi & Stok", "Penjualan, pembelian, mutasi stok dan mutasi kas akan dihapus. Master Produk tetap ada dan stoknya menjadi 0."):
        return
    try:
        engine.dispose()
        with SessionLocal() as session:
...
def _reset_all_business(window):
    if not _confirm_reset(window, "Reset Semua Data Bisnis", "SEMUA produk, kategori, satuan, supplier, pelanggan, penjualan, pembelian, mutasi stok dan mutasi kas akan dihapus."):
        return
    try:
        engine.dispose()
        with SessionLocal() as session:
...
class _PopupController:
    def __init__(self, page, form_box, title, description, buttons=None, width=720, height=520):
        ...
        root.setSpacing(10)
        heading = QLabel(title); heading.setObjectName("popupTitle"); root.addWidget(heading)
        hint = QLabel(description); hint.setObjectName("popupHint"); hint.setWordWrap(True); root.addWidget(hint)
        form_box.setParent(self.dialog); form_box.show(); root.addWidget(form_box, 1)
        actions = QHBoxLayout(); actions.setSpacing(8)
        for button in buttons or []:
            button.setParent(self.dialog); button.show(); actions.addWidget(button)
        actions.addStretch()
        close = QPushButton("Tutup"); close.setObjectName("secondary"); close.clicked.connect(self.dialog.reject); actions.addWidget(close)
